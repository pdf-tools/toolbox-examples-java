...
This kit contains the CustomValidation sample for PdfTools SDK for Java. Pdftools SDK is a development library that lets you integrate PDF processing into your applications. For more information, review the Pdftools [documentation portal](https://www.pdf-tools.com/docs/).
...
3. Right-click the file containing the `main` method and select **Run 'ToolboxCustomValidation'**.
...
### Using Maven
  ...
  ```bash
  mvn clean install
  mvn exec:java -Dexec.mainClass="ToolboxCustomValidation.ToolboxCustomValidation" -Dexec.args="<inputPath> <iniPath> [<pdfPassword>]"
  ```
...
### Using Gradle
  ...
  ```bash
  gradle build
  ./gradlew run --args="<inputPath> <iniPath> [<pdfPassword>]"
  ```
