...
This kit contains the ListInfo sample for PdfTools SDK for Java. Pdftools SDK is a development library that lets you integrate PDF processing into your applications. For more information, review the Pdftools [documentation portal](https://www.pdf-tools.com/docs/).
...
3. Right-click the file containing the `main` method and select **Run 'ToolboxListInfo'**.
...
### Using Maven
  ...
  ```bash
  mvn clean install
  mvn exec:java -Dexec.mainClass="ToolboxListInfo.ToolboxListInfo" -Dexec.args="<inputPath> [<pdfPassword>]"
  ```
...
### Using Gradle
  ...
  ```bash
  gradle build
  ./gradlew run --args="<inputPath> [<pdfPassword>]"
  ```
