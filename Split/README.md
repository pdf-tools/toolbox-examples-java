...
3. Right-click the file containing the `main` method and select **Run 'ToolboxSplit'**.
...
### Using Maven
  ...
  ```bash
  mvn clean install
  mvn exec:java -Dexec.mainClass="ToolboxSplit.ToolboxSplit" -Dexec.args="<inputPath> <firstPage> <lastPage> <outputPath>"
  ```
...
### Using Gradle
  ...
  ```bash
  gradle build
  ./gradlew run --args="<inputPath> <firstPage> <lastPage> <outputPath>"
  ```
