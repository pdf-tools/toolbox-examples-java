...
This kit contains the AddAnnotations sample for PdfTools SDK for Java. Pdftools SDK is a development library that lets you integrate PDF processing into your applications. For more information, review the Pdftools [documentation portal](https://www.pdf-tools.com/docs/).
...
3. Right-click the file containing the `main` method and select **Run 'ToolboxAddAnnotations'**.
...
### Using Maven
  ...
  ```bash
  mvn clean install
  mvn exec:java -Dexec.mainClass="ToolboxAddAnnotations.ToolboxAddAnnotations" -Dexec.args="<inputPath> <outputPath>"
  ```
